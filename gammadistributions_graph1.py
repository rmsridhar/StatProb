...
hstd = np.std(h)
alpha=(hmean/hstd)**2
lambda1=(hstd*hstd)/hmean
print(math.gamma(2.3))
gamma_pdf=[]
for i in range(len(h)):
    lambda_h=-lambda1*h[i]
    exp=math.exp(lambda_h)
    pow_xalpha=pow(h[i],alpha-1)
    pow_lambda=pow(lambda1,alpha)
    gamma_val=math.gamma(alpha)
    gamma_calc=exp*pow_xalpha*pow_lambda/gamma_val
    gamma_pdf.append[gamma_calc]
    
plt.plot(h,gamma_pdf,linewidth=3,color='y')
plt.show()
